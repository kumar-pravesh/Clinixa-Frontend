...
const AppointmentApproval = () => {
    ...
    const handleApprove = (id) => {
        ...
        if (!doctor || doctor.status !== 'active') {
            alert('Γ¥î Cannot approve: Doctor is not available');
            return;
        }
    ...
    const stats = [
        { label: 'Total Appointments', value: appointments.length, icon: '≡ƒôï', color: 'bg-blue-100' },
        { label: 'Pending', value: appointments.filter(a => a.status === 'pending').length, icon: 'ΓÅ│', color: 'bg-yellow-100' },
        { label: 'Approved', value: appointments.filter(a => a.status === 'approved').length, icon: 'Γ£ô', color: 'bg-green-100' },
        { label: 'Rejected', value: appointments.filter(a => a.status === 'rejected').length, icon: 'Γ£ò', color: 'bg-red-100' },
    ];
                            ...
                            <div className={`mb-4 p-3 rounded-lg ${doctors.find(d => d.name === appointment.doctorName)?.status === 'active' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                                <div className="flex items-start space-x-2">
                                    {doctors.find(d => d.name === appointment.doctorName)?.status === 'active' ? (
                                        <>
                                            <span className="text-green-600 text-lg">Γ£ô</span>
                                            <div>
